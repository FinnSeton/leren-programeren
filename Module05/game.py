...
try:
    ...
    if klasse == 'debug':
        ...
        if password == 'Password123':
            Attackoptie = ('Een: Dikke bonker', 'Twee: Nuke', 'Drie: Explosive Boem')
            een = 99999  # Arrow
            twee = 99999  # Piercing Arrow
            drie = 99999  # Explosive Arrow
...
def battle():
    global PlayerHP
    global current_enemyHP
    global armor
    global coins
    PlayerHP = 100
    current_enemyHP = 0
    enemydmg = 0
    if PLAYER.enemy == 'Wolf':
        current_enemyHP = 75
        enemydmg = random.randint(1, 6)
    elif PLAYER.enemy == 'GreenGoblin':
        current_enemyHP = 100
        enemydmg = random.randint(3, 7)
    elif PLAYER.enemy == 'Demon':
        current_enemyHP = 250 
        enemydmg = random.randint(0, 10)
        
    while battlestop is False:  # battle loop
        clear_console()
        print(f'''
        _______________________________________________________________________________________
        Battle:
                        {naam} Hp = {PlayerHP} Armor = {armor}            {PLAYER.enemy} Hp = {current_enemyHP}
        ...
        if attackchoice == 'een':
            current_enemyHP = current_enemyHP - een
            PlayerHP = PlayerHP - enemydmg
        elif attackchoice == 'twee':
            current_enemyHP = current_enemyHP - twee
            PlayerHP = PlayerHP - enemydmg
        elif attackchoice == 'drie':
            current_enemyHP = current_enemyHP - drie
            PlayerHP = PlayerHP - enemydmg
        else:
        ...
        if current_enemyHP <= 0:
            printDelay('Je hebt je tegenstander verslagen je kan nu door lopen')
            coins = coins + random.randint(1, 10)
            input(f'Je hebt {coins} coin')
            PlayerHP  = 100 + armor
            break
###########################################################################
